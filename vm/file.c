...
do_munmap (void *addr) {
    ...
    if(type == 2) {
        ...
        // struct file_page *file_pg = &munmap_page -> file;
        struct list_elem *e;
        // struct lazy_file *arg = (struct lazy_file *)file_pg -> aux;
        struct mmap_file *tmp;
        // struct mmap_file *tmp = arg -> mmapStruct;
        // printf("after get mmap struct \n");
